function [time,boxes,dispStuff]=denBasedPlsDetect(img,opts)
time=tic;
...
for i=1:numel(opts.pDetect.scaleRange)
    ...
    findPLSDisplacement;
    if opts.pDetect.plsForGUI && ~isempty(allPlsBox)
...
if opts.pDetect.plsForGUI && ~isempty(matchScale)
...
%%
    ...
    function match=matchCenterScale
        frameId=opts.dtsetOpts.indexTestFile(opts.pDetect.iFrame);
